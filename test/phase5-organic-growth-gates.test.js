...
test("Phase 5 allows source-backed organic ebook conversion posts", () => {
  const gate = runPhase5OrganicGrowthGate({
    ...
    generated: {
      ...
      content: `For readers trying to understand artificial intelligence without buying the theatre, this book gives a practical way to spot weak claims, ask better questions \
and decide where the tools actually help. It keeps the promise grounded, which is usually where the useful work begins. Readers get a calmer route through the noise before \
they commit time, money or trust.\n\nRead more: ${FEATURED_BOOK.bookUrl}`,
...
test("Phase 5 requires the ebook URL in the main Zernio post, not only first-comment metadata", () => {
  const gate = runPhase5OrganicGrowthGate({
    ...
    generated: {
      ...
      imageUrl: FEATURED_BOOK.coverArtUrl,
      content: "For readers trying to understand artificial intelligence without buying the theatre, this book gives a practical way to spot weak claims, ask better questions \
and decide where tools actually help. The point is useful judgement before committing time, money or trust, with enough practical detail to apply the ideas rather than merely admire them.",
      firstComment: `Featured book: ${FEATURED_BOOK.title}\nRead more: ${FEATURED_BOOK.bookUrl}`,
    },
  });
...
test("Phase 5 does not mistake a negated guarantee for promotional hype", () => {
  const gate = runPhase5OrganicGrowthGate({
    contentType: "organic-visual-social",
    generated: {
      social_caption: "Reliable artificial intelligence performance is not guaranteed. The useful work is monitoring failures, testing assumptions and keeping a human \
checkpoint where the risk warrants one.",
      hashtags: ["#AIReality", "#AIGovernance", "#PracticalAI"],
      imagePrompt: "Create high-impact premium editorial artwork with a dark navy and charcoal base, electric teal and muted coral accents, cinematic strong contrast, no text, \
no letters, no numbers, no logos, no watermarks, portrait 4:5 composition.",
    },
...
test("Phase 5 visual social gate ignores date fragments in generated artefact URLs", () => {
  const gate = runPhase5OrganicGrowthGate({
    ...
    generated: {
      ...
      social_caption: "The latest from the artificial intelligence trenches: chatbots are getting clever enough to weave ads into conversations, making them hard to spot. \
Researchers are flagging this as a plausible trust problem, not a scandal to inflate. The practical point is transparency: users need to know when advice is advice and when it \
is monetised persuasion.",
