...
| `POST` | `/audits/website/run` | audits | Starts the single AIMS-owned website pipeline: Digital Growth -> SEO/AEO/GEO -> Mobile UX -> expert council -> final PDF/HTML/JSON report set -> temporary cleanup -> RAMS website handoff. | Standard audit-run body. | Hookdeck dedupe; AIMS owns all child orchestration. | 202 with parent pipeline job. |
| `GET` | `/audits/website/jobs/:sessionId` | audits | Reads parent website-audit pipeline state and final PDF/HTML/JSON URLs plus RAMS handoff state when complete. | Path `sessionId`. | None | 200 public job or 404. |
...
5. AIMS renders the final report once, publishing exactly three retained representations under `audits/website/YYYY-MM/<pipeline-session>/`: `website-audit.pdf`, `website-audit.html`, and `website-audit.json`.
6. Only after all three final objects publish successfully, AIMS recursively deletes the whole `audits/_tmp/website/<pipeline-session>/` tree and verifies no temporary objects remain. No parent `request.json`, `latest.json`, screenshots or intermediate stage/council files are retained.
7. AIMS then calls RAMS `POST /rebuild/website/run` with the exact `website-audit.json` R2 key and a deterministic idempotency key. RAMS therefore never needs a website `latest.json` pointer.
8. Parent job state is kept in the existing durable job store, outside the final audit artefact prefix, and RAMS handoff can be retried with `POST /audits/website/jobs/:sessionId/rams/retry`.
