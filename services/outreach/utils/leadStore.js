...
export async function saveLeadBatch({ keyword, leads = [], thresholds, generatedAt = new Date().toISOString() } = {}) {
  ...
  const stored = await putPrivateJson("commsHub", key, payload, { cacheControl: "no-store, max-age=0" });
  return { bucketAlias: "commsHub", bucket: stored.bucket, key, uri: stored.uri, count: leads.length };
}
