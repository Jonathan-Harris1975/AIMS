...
const NUMERIC_ENVS = new Set([
  ...
  "BLOTATO_VIDEO_POLL_MAX_DURATION_MS",
  ...
  "BLOTATO_NEWS_MIN_SCRIPT_WORDS",
  "BLOTATO_NEWS_TARGET_SCRIPT_WORDS",
  "BLOTATO_NEWS_MAX_SCRIPT_WORDS",
  "BLOTATO_NEWS_MIN_SCENE_WORDS",
  "BLOTATO_RENDERED_QA_THRESHOLD",
  "BLOTATO_RENDERED_QA_FRAMES",
  "BLOTATO_RENDERED_MIN_SECONDS",
  ...
  "COMMS_HUB_MAX_WEBHOOK_BYTES",
  "COMMS_HUB_JOTFORM_TIMEOUT_MS",
  "COMMS_HUB_D1_TIMEOUT_MS",
  "COMMS_HUB_PROVIDER_RETRY_ATTEMPTS",
  "COMMS_HUB_PROVIDER_RETRY_BASE_MS",
  "COMMS_HUB_PROVIDER_RETRY_MAX_MS",
  "COMMS_HUB_ARCHIVE_POLL_MS",
  "COMMS_HUB_ARCHIVE_BATCH_SIZE",
  "COMMS_HUB_ARCHIVE_LEASE_MS",
  "COMMS_HUB_ARCHIVE_MAX_ATTEMPTS",
  "COMMS_HUB_MONTH_END_ARCHIVE_POLL_MS",
  "COMMS_HUB_MONTH_END_ARCHIVE_BATCH_SIZE",
  "COMMS_HUB_ZERNIO_WEBHOOK_RECONCILE_INTERVAL_MS",
  "COMMS_HUB_RUNTIME_SUPERVISOR_MAX_RETRY_MS",
  "COMMS_HUB_RUNTIME_SUPERVISOR_RETRY_MS",
]);
...
const BOOLEAN_ENVS = new Set([
  ...
  "ZERNIO_VALIDATE_TARGET_ACCOUNTS",
  "BLOTATO_TEMPLATE_VERIFY",
  "BLOTATO_TEMPLATE_AUTO_DISCOVERY",
  "BLOTATO_NEWS_JSON_RESPONSE_FORMAT",
  "BLOTATO_RENDERED_QA_ENABLED",
  "BLOTATO_RENDERED_QA_REQUIRED",
  "BLOTATO_RENDERED_QA_BLOCK_SOFT_FAILURES",
  "BLOTATO_RENDERED_QA_BLOCK_HARD_FAILURES",
  ...
  "COMMS_HUB_ENABLED",
  "COMMS_HUB_ARCHIVE_WORKER_ENABLED",
  "COMMS_HUB_MONTH_END_ARCHIVE_ENABLED",
  "COMMS_HUB_ZERNIO_WEBHOOK_RECONCILE_ENABLED",
  "COMMS_HUB_RUNTIME_SUPERVISOR_ENABLED",
]);
...
function validateSecretReference({ key, value, line }, errors) {
  ...
  if (!SECRET_REFERENCE_PATTERN.test(cleaned)) {
    errors.push({
      ...
      key,
      message:
        `Invalid Koyeb secret reference for ${key}; use {{ secret.SECRET_NAME }} or {{secret.SECRET_NAME}} with letters, numbers, underscores or hyphens only`,
    });
  }
}
...
function validateNumber({ key, value, line }, errors) {
  ...
  if (["BLOTATO_RENDERED_QA_THRESHOLD", "ARTWORK_VISUAL_QA_THRESHOLD"].includes(key) && (number < 1 || number > 100)) {
    errors.push({ line, key, message: `${key} must be between 1 and 100` });
  }

  if (key === "BLOTATO_RENDERED_QA_FRAMES" && (number < 4 || number > 12)) {
    errors.push({ line, key, message: `${key} must be between 4 and 12` });
  }

  if (["BLOTATO_RENDERED_MIN_SECONDS", "BLOTATO_RENDERED_MAX_SECONDS"].includes(key) && (number < 30 || number > 60)) {
    errors.push({ line, key, message: `${key} must be between 30 and 60` });
  }

  if (["BLOTATO_NEWS_MIN_SCRIPT_WORDS", "BLOTATO_NEWS_TARGET_SCRIPT_WORDS", "BLOTATO_NEWS_MAX_SCRIPT_WORDS", "BLOTATO_NEWS_MIN_SCENE_WORDS"].includes(key) && (number < 60 || number > 180)) {
    errors.push({ line, key, message: `${key} must be between 60 and 180` });
  }
}
