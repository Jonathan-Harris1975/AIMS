...
test("Blotato shorts enforce the 35-55 second finished range", async () => {
...
test("production word-count range is calibrated for the 35-55 second contract", async () => {
  ...
  assert.match(env, /^BLOTATO_VIDEO_SCENE_COUNT=5$/m);
});
