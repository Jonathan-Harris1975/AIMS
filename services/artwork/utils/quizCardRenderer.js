...
function optionHtml(option, correctLetter, answerCard) {
  ...
  return `<div class="${className}"><div class="letter">${esc(option.letter)}</div><div class="option-text">${esc(option.text)}</div>${isCorrect ?
     '<div class="tick" aria-label="Correct">✓</div>' : ""}</div>`;
}
...
export function buildQuizCardHtml(card = {}) {
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><style>
    *{box-sizing:border-box}html,body{width:1080px;height:1080px;margin:0;overflow:hidden}body{font-family:Arial,Helvetica,sans-serif;background:#071426;color:#f8fafc}
    .card{position:relative;width:1080px;height:1080px;padding:68px 72px 58px;display:flex;flex-direction:column;background:radial-gradient(circle at 88% 7%,rgba(22,180,173,.\
25),transparent 34%),radial-gradient(circle at 6% 96%,rgba(251,146,60,.18),transparent 34%),linear-gradient(150deg,#071426,#10233d 58%,#0b1729)}
    ...
    .option{min-height:112px;display:grid;grid-template-columns:68px 1fr 48px;align-items:center;gap:18px;padding:18px 24px;border-radius:22px;border:2px solid rgba(255,255,\
255,.18);background:rgba(255,255,255,.075);box-shadow:0 12px 30px rgba(0,0,0,.14)}
    .option-muted{opacity:.68}.option-correct{opacity:1;border-color:#5eead4;background:linear-gradient(90deg,rgba(13,148,136,.72),rgba(20,184,166,.36));box-shadow:0 0 0 4px \
rgba(94,234,212,.12),0 16px 35px rgba(0,0,0,.2)}
    ...
    .explanation{margin-top:18px;padding:20px 24px;border-radius:20px;background:rgba(2,6,23,.5);border:1px solid rgba(255,255,255,.15);font-size:${explanationSize}px;line-\
height:1.28;color:#e2e8f0;max-height:142px;overflow:hidden}
    .footer{margin-top:auto;padding-top:20px;display:flex;align-items:center;justify-content:space-between;font-size:27px;font-weight:800;color:#fdba74}.brand{font-size:19px;\
letter-spacing:.08em;text-transform:uppercase;color:#94a3b8}
  </style></head><body><main class="card"><div class="kicker">${esc(title)}</div><h1>${esc(question)}</h1><section class="options">${options.map((option) => optionHtml(option,
     correctLetter, answerCard)).join("")}</section>${answerCard && explanation ? `<section class="explanation">${esc(explanation)}</section>` :
        ""}<footer class="footer"><span>${esc(footer)}</span><span class="brand">Jonathan Harris</span></footer></main></body></html>`;
}
