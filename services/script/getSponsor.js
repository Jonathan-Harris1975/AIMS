...
function resolveCoverArtUrl(book = {}) {
  return (
    cleanString(book?.social_post_image_url) ||
    cleanString(book?.social_image_url) ||
    cleanString(book?.social_portrait_image_url) ||
    cleanString(book?.cover_art_url) ||
    cleanString(book?.main_image) ||
    cleanString(book?.cover) ||
    cleanString(book?.image_url) ||
    ""
  );
}

function resolveManuscriptUrl(book = {}) {
  return (
    cleanString(book?.manuscript_url) ||
    cleanString(book?.manuscript_pdf_url) ||
    cleanString(book?.pdf_url) ||
    cleanString(book?.google_drive_pdf_url) ||
    ""
  );
}
...
export function buildSponsorFromPayload(payload) {
  ...
  return {
    ...
    coverArtUrl: resolveCoverArtUrl(book),
    socialImageUrl: cleanString(book.social_post_image_url) || cleanString(book.social_image_url) || "",
    manuscriptUrl: resolveManuscriptUrl(book),
...
export { DEFAULT_FEATURED_BOOK_API_URL, FALLBACK_SPONSOR, resolveSponsorUrl, resolveCoverArtUrl, resolveManuscriptUrl };
