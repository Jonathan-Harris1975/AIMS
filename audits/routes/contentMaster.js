...
router.post("/run",hookdeckDedupe("audits:content-master:run"),asyncRoute(async(req,res)=>{const job=await startAsyncAuditRouteJob({auditType:AUDIT_TYPE,payload:req.body||{},req,runner:runContentMasterAudit,metadata:{route:"audits.content-master.run"}});return res.status(202).json(job);}));
