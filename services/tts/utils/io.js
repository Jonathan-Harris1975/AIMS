...
import { putObject, uploadPrivateBuffer, putPrivateJson, R2_BUCKET_RAW_AUDIO } from "../../shared/utils/r2-client.js";
...
export async function saveTtsChunk(audioBuffer, key) {
  ...
  await uploadPrivateBuffer(RAW_BUCKET, key, audioBuffer, "audio/mpeg");
...
export async function saveMergedTts(audioBuffer, key) {
  ...
  await uploadPrivateBuffer(MERGED_BUCKET, key, audioBuffer, "audio/mpeg");
...
export async function saveTtsMeta(key, data) {
  ...
  await putPrivateJson(META_BUCKET, key, data);
