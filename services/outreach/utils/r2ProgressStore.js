 ...
 * Keep this minimal – this is orchestration state, not analytics
 */
...
export async function loadProgress() {
  ...
  try {
    const txt = await getObjectAsText("meta-system", key);
    ...
    // First run or missing file – this is expected behaviour
...
/**
 * Persist progress to R2 metasystem bucket
 */
export async function saveProgress(progress) {
  ...
  try {
    await putJson("meta-system", key, payload);
